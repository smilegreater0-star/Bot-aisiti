...
def senanan_idm_hunt(client: Groq, model: str, raw_data: dict,
    ...
    notify_ctx = _build_notify_ctx(ctx)
    state_ctx  = _build_json_ctx(ctx)
    # Ekstrak SH/SL dari state untuk disertakan langsung di prompt
    import re as _re
    sh_match = _re.search(r'SH=([0-9.]+)', state_ctx)
    sl_match = _re.search(r'SL=([0-9.]+)', state_ctx)
    sh_str = sh_match.group(1) if sh_match else str(sh)
    sl_str = sl_match.group(1) if sl_match else str(sl)
    price  = raw_data.get("price", 0)
    prompt = _ai_cfg.build_prompt("senanan", {
        "state_ctx":    state_ctx,
        "candle_table": (
            f"Format: idx|Open|High|Low|Close (baca kiri ke kanan)\n"
            f"Harga sekarang: {price} | SH={sh_str} | SL={sl_str}\n"
            f"Cari IDM HANYA di antara SH dan SL.\n"
            f"CANDLE M5 ({candle_limit} terakhir):\n{m5_table}"
        ),
        "notify_ctx":   notify_ctx,
    })
    if not prompt:
        prompt = (
            f"Kamu Senanan, IDM M5 hunter ICT. Harga={price} SH={sh_str} SL={sl_str}.\n"
            f"Candle M5 (idx|O|H|L|C):\n{m5_table}\n"
            f"Cari IDM bullish M5 di range SH-SL. BALAS HANYA JSON:\n"
            f'{{"analysis":"...","decision":"idm_found|wait","idm_found":false,'
            f'"watch_level":0,"actions":[],"next_phase":"bos_guard","chat_msg":"..."}}'
        )
    ...
    if not parsed:
        ...
        return {"idm_found": False, "watchlist": [], "chat_msg": "Senanan: parse gagal", "confidence": 0}
