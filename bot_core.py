...
class BotCore:
    ...
    def _check_stuck(self) -> bool:
        ...
        if self._cycles_in_phase > max_c:
            ...
            if self._phase == "fvg_wait":
                ...
                # IDM hunt sudah terlalu lama → force h1_scan, scan BOS baru
                logger.warning(f"[STUCK] idm_hunt {self._cycles_in_phase} siklus → force h1_scan")
                self._reset("stuck_idm_hunt_too_long")
                return True
            elif self._phase == "bos_guard":
                self._set_phase("idm_hunt", "stuck bos_guard, reset IDM", "stuck_detector")
                self._senanan_data = {}
                self._shina_data   = {}
                self.watchlist.clear_untriggered()
    ...
    def _run_idm_hunt(self, raw_data: dict):
        ...
        if not result:
            ...
            # Tetap push status ke live feed supaya user tahu bot aktif
            api_server.push_live_msg("ai2", "Senanan",
                f"🔍 IDM hunting... harga {price:.5f} range SH={sh:.5f}–SL={sl_lvl:.5f}",
                self.symbol)
            return
